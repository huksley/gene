  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (isRetriableStatus(res.status) && attempt < retries) {
        logger.warn(`${logger.tag.fetch} fetch ${url} → HTTP ${res.status}; retrying (${attempt + 1}/${retries})`);
        continue;
      }
      return res;
    } catch (error) {
      ...
      if (attempt < retries) {
        ...
        logger.warn(`${logger.tag.fetch} fetch ${url} failed (${reason}); retrying (${attempt + 1}/${retries})`);
        continue;
      }
    }
  }
